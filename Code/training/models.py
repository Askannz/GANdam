...
def make_generator():
    ...
    reshape = Reshape((1, 1, LATENT_DIM))(input_tensor)  # 1x1

    conv1 = Conv2DTranspose(2048, kernel_size=3, strides=2, padding="same", activation="relu")(reshape)  # 2x2
    batch1 = BatchNormalization(momentum=BN_MOMENTUM)(conv1)

    conv2 = Conv2DTranspose(2048, kernel_size=3, strides=2, padding="same", activation="relu")(batch1)  # 4x4
    batch2 = BatchNormalization(momentum=BN_MOMENTUM)(conv2)

    conv3 = Conv2DTranspose(1024, kernel_size=3, strides=2, padding="same", activation="relu")(batch2) # 8x8
    batch3 = BatchNormalization(momentum=BN_MOMENTUM)(conv3)

    conv4 = Conv2DTranspose(512, kernel_size=3, strides=2, padding="same", activation="relu")(batch3) # 16x16
    batch4 = BatchNormalization(momentum=BN_MOMENTUM)(conv4)

    conv5 = Conv2DTranspose(256, kernel_size=3, strides=2, padding="same", activation="relu")(batch4) # 32x32
    batch5 = BatchNormalization(momentum=BN_MOMENTUM)(conv5)

    conv6 = Conv2DTranspose(128, kernel_size=3, strides=2, padding="same", activation="relu")(batch5) # 64x64
    batch6 = BatchNormalization(momentum=BN_MOMENTUM)(conv6)

    conv7 = Conv2DTranspose(64, kernel_size=3, strides=2, padding="same", activation="relu")(batch6) # 128x128
...
def make_discriminator():
    ...
    conv1 = Conv2D(32, kernel_size=3, strides=2, padding="same")(input_tensor)  # 128x128
    ...
    conv2 = Conv2D(64, kernel_size=3, strides=2, padding="same")(dropout1)  # 64x64
    ...
    conv3 = Conv2D(128, kernel_size=3, strides=2, padding="same")(dropout2)  # 32x32
    ...
    conv4 = Conv2D(256, kernel_size=3, strides=2, padding="same")(dropout3)  # 16x16
    ...
    conv5 = Conv2D(512, kernel_size=3, strides=2, padding="same")(dropout4)  # 8x8
    activ5 = LeakyReLU(alpha=0.2)(conv5)
    dropout5 = Dropout(0.25)(activ5)

    conv6 = Conv2D(1024, kernel_size=3, strides=2, padding="same")(dropout5)  # 4x4
    activ6 = LeakyReLU(alpha=0.2)(conv6)
    dropout6 = Dropout(0.25)(activ6)

    conv7 = Conv2D(2048, kernel_size=3, strides=2, padding="same")(dropout6)  # 2x2
    activ7 = LeakyReLU(alpha=0.2)(conv7)
    dropout7 = Dropout(0.25)(activ7)

    conv8 = Conv2D(2048, kernel_size=3, strides=2, padding="same")(dropout7)  # 1x1
